...
function buildTerrain() {
  ...
  const makeBox = (x, y, z, hx, hy, hz, color, rotY = 0) => {
    ...
    world.createCollider(RAPIER.ColliderDesc.cuboid(hx, hy, hz).setFriction(1.35), body);
...
function createQuadRobot() {
  ...
  bodies.push(torso);
  world.createCollider(RAPIER.ColliderDesc.cuboid(0.36, 0.09, 0.2).setDensity(1.2).setFriction(1.2), torso);
  addBodyMesh(torso, new THREE.Mesh(new THREE.BoxGeometry(0.72, 0.18, 0.4), new THREE.MeshStandardMaterial({ color: '#22364d' })));
  ...
  for (const leg of legDefs) {
    const upper = world.createRigidBody(
      RAPIER.RigidBodyDesc.dynamic().setTranslation(leg.x, 0.97, leg.z).setLinearDamping(0.22).setAngularDamping(0.5)
    );
    const lower = world.createRigidBody(
      RAPIER.RigidBodyDesc.dynamic().setTranslation(leg.x, 0.73, leg.z).setLinearDamping(0.22).setAngularDamping(0.5)
    );
    bodies.push(upper, lower);

    world.createCollider(RAPIER.ColliderDesc.capsule(0.07, 0.045).setDensity(0.7), upper);
    world.createCollider(RAPIER.ColliderDesc.capsule(0.08, 0.04).setDensity(0.65).setFriction(1.4), lower);
    world.createCollider(RAPIER.ColliderDesc.ball(0.05).setTranslation(0, -0.11, 0).setFriction(2.6), lower);
    ...
    hip.configureMotorModel(RAPIER.MotorModel.ForceBased);
    knee.configureMotorModel(RAPIER.MotorModel.ForceBased);
  ...
  return {
    ...
    step(dt) {
      ...
      gaitPhase += dt * (1.6 + Math.abs(command.forward) * 1.6 + Math.abs(command.turn) * 0.6) * boost;
      const stride = (0.22 + Math.abs(command.forward) * 0.48) * boost;
      const kneeLift = 0.78 + Math.abs(command.forward) * 0.42;
      const stiff = 58 + Math.abs(command.forward) * 48;
      const damp = 12 + Math.abs(command.forward) * 8;

      for (const leg of legs) {
        const cyc = gaitPhase * Math.PI * 2 + leg.phase + command.turn * (leg.side > 0 ? 0.25 : -0.25);
      ...
      const push = 10.8 * command.forward * boost;
      torso.applyImpulse({ x: fwd.x * push * dt, y: 0, z: fwd.z * push * dt }, true);
      torso.applyTorqueImpulse({ x: 0, y: command.turn * (0.38 + Math.abs(command.forward) * 0.1) * boost * dt, z: 0 }, true);
...
function createHumanoidRobot() {
  ...
  for (const leg of legsDef) {
    const upper = world.createRigidBody(
      RAPIER.RigidBodyDesc.dynamic().setTranslation(leg.x, 1.02, leg.z).setLinearDamping(0.25).setAngularDamping(0.52)
    );
    const lower = world.createRigidBody(
      RAPIER.RigidBodyDesc.dynamic().setTranslation(leg.x, 0.72, leg.z).setLinearDamping(0.25).setAngularDamping(0.52)
    );
    bodies.push(upper, lower);
    ...
    knee.configureMotorModel(RAPIER.MotorModel.ForceBased);
    joints.push(hip, knee);
    legs.push({ hip, knee, phase: leg.phase });
  }
  ...
  return {
    ...
    step(dt) {
      ...
      gaitPhase += dt * (1.2 + Math.abs(command.forward) * 1.4) * boost;
      const stride = (0.22 + Math.abs(command.forward) * 0.36) * boost;
      const stiff = 64;
      const damp = 13;
      ...
      qTmp.set(rot.x, rot.y, rot.z, rot.w);
      const fwd = vTmp.set(1, 0, 0).applyQuaternion(qTmp).setY(0).normalize();
      torso.applyImpulse({ x: fwd.x * 8.4 * command.forward * dt, y: 0, z: fwd.z * 8.4 * command.forward * dt }, true);
      torso.applyTorqueImpulse({ x: 0, y: command.turn * 0.34 * dt, z: 0 }, true);
...
function setMode(nextMode) {
  ...
  if (mode === 'isaac') {
    hintEl.textContent = 'MuJoCo-style tuning: fixed-step + filtered control + contact metrics';
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
  } else if (mode === 'mobile') {
    hintEl.textContent = 'Mobile-priority: large touch controls, stable follow camera';
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5));
  } else {
    hintEl.textContent = 'Humanoid-priority: biped balance walk with stronger PD gains';
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
  }
}
...
function updatePanels() {
  ...
  const groundDist = hit ? hit.timeOfImpact : 3.0;

  const bodyCount = syncEntries.length;
  physicsStatsEl.textContent = `dt ${(SIM_DT * 1000).toFixed(1)}ms | substeps ${lastSubsteps} | rigid ${bodyCount} | mode ${mode}`;
  sensorStatsEl.textContent = `speed ${speed.toFixed(2)} m/s | pitch ${pitch.toFixed(1)} | roll ${roll.toFixed(1)} | ground ${groundDist.toFixed(2)}m`;
  cameraStatsEl.textContent = `cmd fwd ${command.forward.toFixed(2)} turn ${command.turn.toFixed(2)} | cam (${camera.position.x.toFixed(1)}, ${camera.position.y.toFixed(1)}, ${camera.position.z.toFixed(1)})`;
}

function loop() {
  const frameDt = Math.min(clock.getDelta(), 1 / 20);

  const alpha = 1 - Math.exp(-frameDt / CMD_TAU);
  command.forward += (input.forward - command.forward) * alpha;
  command.turn += (input.turn - command.turn) * alpha;

  accumulator += frameDt;
  let substeps = 0;
  while (accumulator >= SIM_DT && substeps < MAX_SUBSTEPS) {
    if (robot) robot.step(SIM_DT);
    world.step();
    accumulator -= SIM_DT;
    substeps += 1;
  }
